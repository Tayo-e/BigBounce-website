...
function HighlightReel() {
  ...
  return <section className="event-reel" id="event-reel" ref={scope}><div className="reel-heading"><Eyebrow>The highlight reel</Eyebrow><h2>We have been<br /><em>at the door.</em></h2><p>Real moments. Real rooms. A considered presence when it matters.</p></div><div className="reel-rail" tabIndex="0" aria-label="Event highlights. Swipe or scroll horizontally to browse.">{reelEvents.map(([number, title, image, caption]) => <article className="reel-card" key={number} tabIndex="0"><img src={image} alt={`${title} event supported by Big Bounce Events`} loading="lazy" /><div className="reel-overlay" /><div className="reel-caption"><span>{number}</span><h3>{title}</h3><p>{caption}</p><ArrowRight /></div></article>)}</div><p className="reel-hint"><span className="reel-hint-desktop">Scroll to explore</span><span className="reel-hint-mobile">Swipe to explore</span><ArrowDownRight size={16} /></p></section>
}
